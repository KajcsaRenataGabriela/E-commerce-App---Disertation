...
class ProfilesApi {
  ...
  Future<void> generateVoucher(String uid, String title, int discount) async {
    ...
    final CollectionReference profilesCollection = _firestore.collection('profiles');

    try {
      // Step 1: Get the reference to the user's document based on the UID.
      final QuerySnapshot<Object?> querySnapshot = await profilesCollection
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        await userDocRef.update(<String, dynamic>{
          'vouchers': FieldValue.arrayUnion([voucher.toJson()]),
        });
